...
def good_review():
    return {
        ...
        "strengths": [
            {"text": "The transporter-law characterization and Reynolds averaging construction give concrete equivariant maps.", "anchors": ["THM-0001"]},
            {"text": "The hierarchy distinguishes anonymous message passing from source-labeled pair-state models.", "anchors": ["TXT-0061"]},
        ],
        "weaknesses": [
            {
                "id": "reviewer-r2-W1",
        ...
        "scores": {"soundness": 3, "presentation": 3, "significance": 2, "originality": 3, "overall": 3},
        "key_questions": [
            {"id": "reviewer-r2-Q1", "question": "How stable is the reported empirical gain across seeds and matched-capacity baselines?", "possible_score_impact": "A robust comparison could raise Significance and Overall by one point."}
        ],
...
def test_persona_compiler_builds_four_diverse_reviewers_for_34584():
    ...
    assert {persona["reviewer_id"] for persona in panel["personas"]} == {"reviewer-r1", "reviewer-r2", "reviewer-r3", "reviewer-r4"}
    assert {assignment["target_type"] for assignment in panel["deep_audit_assignments"]} >= {"mathematical_theory", "empirical_evaluation", "closest_literature", "reproducibility"}
...
def test_coverage_gap_adds_fifth_reviewer():
    dossier = load(DOSSIER)
    dossier["ethics_note"] = "The study involves privacy and sensitive attributes requiring special ethics expertise."
    panel = persona_compiler.compile_panel(dossier, "34584-ethics", load(SCHEMAS / "persona.schema.json"))
    assert panel["gate"]["passed"] is True
    assert len(panel["personas"]) == 5
    ethics = next(item for item in panel["deep_audit_assignments"] if item["target_type"] == "ethics_security")
...
def test_runtime_preserves_identity_persona_and_append_only_scores(tmp_path: Path):
    ...
    second = reviewer_runtime.append_score_history(
        history_path,
        phase="followup",
        scores={"soundness": 3, "presentation": 3, "significance": 3, "originality": 3, "overall": 4},
        confidence=4,
    ...
    with pytest.raises(ValueError, match="frozen reviewer persona changed"):
        reviewer_runtime.initialize_workspace(workspace, "run-34584", "reviewer-r2", persona_compiler.base_personas()[0])
...
def test_queue_runs_one_task_and_checker_reopens(tmp_path: Path):
    ...
    queue_path.write_text((ROOT / "roles/reviewer/phases/followup/tasks.template.json").read_text(), encoding="utf-8")
    ...
    reviewer_runtime.finish_task(queue_path, first["id"], passed=False, feedback="missing one concern")
...
def test_visibility_manifests_enforce_phase_boundaries(tmp_path: Path):
    ...
    for phase in reviewer_runtime.PHASES:
        ...
        completed = subprocess.run(
            [str(ADAPTER), "generate-manifest", "--repo-root", str(ROOT), "--workspace", str(workspace), "--agent-id", "reviewer-r2", "--role", "reviewer", "--phase", phase, "--output", str(manifest)],
            text=True,
            capture_output=True,
            timeout=20,
        )
...
def test_review_checker_passes_anchored_review_and_reopens_bad_artifact():
    ...
    result = review_checker.check_review(review, schema, load(ANCHORS), PAPER.read_text(encoding="utf-8"), ledger)
    assert result == {"passed": True, "action": "complete", "feedback": []}
    domain_language = json.loads(json.dumps(review))
    domain_language["strengths"][0]["text"] += " Reynolds averaging preserves equivariance in the audited finite-group case."
    ...
    rejected = review_checker.check_review(bad, schema, load(ANCHORS), PAPER.read_text(encoding="utf-8"), ledger)
    assert rejected["action"] == "reopen"
    assert {item["code"] for item in rejected["feedback"]} >= {"abstract_copy", "unresolved_anchor", "ledger_mismatch"}
...
def test_phase_templates_and_role_schemas_validate():
    ...
    for path in (ROOT / "roles/reviewer/phases").glob("*/tasks.template.json"):
        errors = list(Draft202012Validator(phase_schema, format_checker=FormatChecker()).iter_errors(load(path)))
...
def test_real_34584_reviewer_fixtures_and_followup_are_valid():
    ...
    assert evidence["official_review_hash"] == "sha256:" + hashlib.sha256((fixture / "official-review.json").read_bytes()).hexdigest()
    assert evidence["concern_ledger_hash"] == "sha256:" + hashlib.sha256((fixture / "concern-ledger.json").read_bytes()).hexdigest()
    assert evidence["score_history_hash"] == "sha256:" + hashlib.sha256((fixture / "score-history.json").read_bytes()).hexdigest()
    ...
    assert {item["concern_id"] for item in load(fixture / "followup.json")["concern_resolutions"]} == concern_ids
...
def test_real_four_reviewer_followups_cover_their_original_concerns():
    ...
    for index in range(1, 5):
        ...
        assert {
            item["status"] for item in followup["concern_resolutions"]
        } <= allowed_statuses
