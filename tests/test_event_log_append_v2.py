...
class EventLogAppendV2Test(unittest.TestCase):
    ...
    def test_exact_retry_is_duplicate_and_changed_draft_conflicts(self) -> None:
        ...
        with self.assertRaises(authority.EventConflictError):
            authority.append_draft(self.draft(payload={"ordinal": "changed"}), self.path, self.run_id)
    ...
    def test_durable_tip_capture_keeps_the_append_lock(self) -> None:
        ...
        first = threading.Thread(target=lambda: (authority.append_draft(self.draft("1"), self.path, self.run_id), first_complete.set()))
        first.start()
        self.assertTrue(entered.wait(2))
        second = threading.Thread(target=lambda: (authority.append_draft(self.draft("2"), self.path, self.run_id), second_complete.set()))
        second.start()
    ...
    def test_holder_death_releases_stable_inode_lock(self) -> None:
        ...
        holder = subprocess.Popen([sys.executable, "-c", source, lock_path], stdout=subprocess.PIPE, text=True)
        assert holder.stdout is not None
        self.assertEqual(holder.stdout.readline().strip(), "locked")
        holder.kill()
        holder.wait(timeout=2)
        self.assertEqual(authority.append_draft(self.draft(), self.path, self.run_id)["status"], "appended")
    ...
    def test_cli_returns_single_json_result(self) -> None:
        ...
        completed = subprocess.run(
            [sys.executable, str(ROOT / "shared" / "event_log_append_v2.py"), str(draft_path), str(self.path), self.run_id],
            check=True,
            capture_output=True,
            text=True,
        )
