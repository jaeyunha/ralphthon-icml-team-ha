...
describe("NdjsonProjectorV2", () => {
  test("atomically saves canonical event, read model, batch ledger, registry, and cursor", async () => {
    ...
    const result = await new NdjsonProjectorV2(store, { publicationRows: (item) => [{ publicationId: "note-1", eventId: item.event.id, eventHash: item.envelope.event_hash, receiptHash: hash("r"), contentHash: hash("content"), sanitizationReceiptHash: hash("sanitization"), audience: "public", releaseStatus: "final", sanitizationStatus: "sanitized_public" }] }).projectCapturedBatch(batch());
